...
public class ReliabilityTestType implements Results {
    ...
    public ReliabilityTestType(UtilityConfig utilityConfig, InfluxDBConfig influxDBConfig,VictoriaMetricsConfig victoriaMetricsConfig) {
    ...
    public String createTableForResults(Long len, String path, @NotNull Test testExemplar) {
        ...
        Map<String, LocalDateTime> times = testUtility.calclateTimestampsForGatling(testExemplar.getEndTime(), testExemplar.getStepDuration());
        Map<String, String> timestamps = testUtility.convertToTimestampInflux(times.get("startTime"), times.get("endTime"));
       // InfluxDTO model = new InfluxDTO(timestamps.get("startTimestamp"), timestamps.get("endTimestamp"), influxDBConfig, utilityConfig);
        ...
        sb.append("<h2><b>Результаты нагрузочного тестирования</b></h2>")
                .append("<table>")
                .append("<tr>" + "<th colspan=\"9\" style=\"text-align: center;\">Уровень нагрузки от профиля: ").append(testExemplar.getStartPercent())
                ...
                .append(" </th>").append("</tr>")
                .append("<tr><th>№</th><th>Описание сценария</th><th>Название скрипта</th><th>Целевая интенсивность, операций/час</th><th>Количество успешных операций</th><th>Количество неуспешных операций</th><th>Точность</th><th>90 перцентиль времени отклика, ms</th><th>SLA по времени отклика, сек</th></tr>");
